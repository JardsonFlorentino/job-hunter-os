...
import { ApplicationEventType, ApplicationStatus, JobStatus, ReplyClassification } from "@prisma/client";
import { ImapFlow } from "imapflow";
import { simpleParser, type ParsedMail } from "mailparser";

import { callAi } from "../ai/client.js";
import { prisma } from "../prisma/client.js";
import { detectFraudSignals, parseReplyClassification, REPLY_CLASSIFICATION_PROMPT } from "./reply-classification.js";
...
function getErrorDetail(error: unknown, property: "response" | "command"): unknown {
...
function getReferenceIds(mail: ParsedMail): string[] {
  ...
  return [mail.inReplyTo, ...references].filter(
  ...
  try {
    ...
    try {
      for await (const message of client.fetch(
        ...
        try {
          if (!message.source) {
            throw new Error("O servidor IMAP não retornou o conteúdo do e-mail.");
          }
          ...
          if (!jobId) {
            const alertCount = await ingestOwnedJobAlert(prisma, { subject: parsedMail.subject ?? null, text: getReadableText(parsedMail), html: typeof parsedMail.html === "string" ? parsedMail.html : null }, callAi);
            if (alertCount > 0) {
              await client.messageFlagsAdd(message.uid, ["\\Seen"], { uid: true });
              console.info(`[IMAP] Alerta UID ${message.uid}: ${alertCount} vagas públicas persistidas.`);
              continue;
            }
            console.info(
          ...
          const classification = fraudSignals.length > 0
            ? ReplyClassification.SUSPEITA_FRAUDE
            : ReplyClassification[(parseReplyClassification(await callAi(`Classifique o e-mail abaixo:\n\n${emailText}`, REPLY_CLASSIFICATION_PROMPT, { json: true }))).classification];
          const relatedJob = await prisma.job.findUnique({ where: { id: jobId }, include: { opportunity: { include: { applications: { orderBy: { created_at: "desc" }, take: 1 } } } } });
          ...
          await prisma.inboxMessage.upsert({
            where: { message_id: parsedMail.messageId ?? `imap:${getRequiredEnvironmentVariable("IMAP_USER")}:${message.uid}` },
            create: { profile_id: profile.id, application_id: application?.id ?? null, message_id: parsedMail.messageId ?? `imap:${getRequiredEnvironmentVariable("IMAP_USER")}:${message.uid}`, sender, subject: parsedMail.subject ?? null, body_text: emailText, classification, fraud_signals: fraudSignals, received_at: parsedMail.date ?? new Date() },
            update: {},
          });

          const jobStatus = classification === ReplyClassification.ENTREVISTA ? JobStatus.ENTREVISTA : classification === ReplyClassification.TESTE ? JobStatus.TESTES : classification === ReplyClassification.REJEICAO ? JobStatus.REJEITADO : null;
          if (jobStatus) await prisma.job.update({ where: { id: jobId }, data: { status: jobStatus } });
          const applicationStatus = classification === ReplyClassification.ENTREVISTA ? ApplicationStatus.INTERVIEW : classification === ReplyClassification.TESTE ? ApplicationStatus.TEST : classification === ReplyClassification.REJEICAO ? ApplicationStatus.REJECTED : null;
          if (application && applicationStatus) {
            await prisma.$transaction([
              prisma.application.update({ where: { id: application.id }, data: { status: applicationStatus, next_action: classification === ReplyClassification.ENTREVISTA ? "Preparar entrevista" : classification === ReplyClassification.TESTE ? "Concluir teste técnico" : null } }),
              prisma.applicationEvent.create({ data: { application_id: application.id, type: ApplicationEventType.REPLY_RECEIVED, from_status: application.status, to_status: applicationStatus, message: `Resposta classificada como ${classification}.` } }),
            ]);
            if (applicationStatus === ApplicationStatus.INTERVIEW) {
              const existingInterview = await prisma.interview.findFirst({ where: { application_id: application.id }, select: { id: true } });
              if (!existingInterview) await prisma.interview.create({ data: { application_id: application.id, stage: "Entrevista inicial", contact_email: sender, notes: "Criada automaticamente a partir de resposta classificada; revisar data e detalhes." } });
            }
          }
    ...
    throw new Error(`Não foi possível consultar a caixa de entrada: ${message}`);
  } finally {
    if (client.usable) {
      try {
        await client.logout();
